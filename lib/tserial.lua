...
function Tserial.pack(t, drop, indent)
    ...
    local function proc(k,v, omitKey)   -- encode a key/value pair
        ...
        elseif tk == "string" then local f = string.format("%q",k); k = '['..f..']'
        --elseif tk == "string" then local f = string.format("%q",k) if f ~= '"'..k..'"' then k = '['..f..']' end -- breaks when index contains special characters
